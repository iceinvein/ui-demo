		...
		const animate = () => {
			...
			for (const orb of orbsRef.current) {
				...
				if (mouseRef.current.down) {
					...
					if (dist > 1) {
						orb.vx +=
							(dx / dist) * ATTRACTION_STRENGTH * Math.min(100, 5000 / dist);
						orb.vy +=
							(dy / dist) * ATTRACTION_STRENGTH * Math.min(100, 5000 / dist);
					}
				}
	...
	return (
		<div className="absolute inset-0 overflow-hidden bg-black">
			...
			>
				...
				<p className="text-sm text-white/40">Click anywhere to begin</p>
			</motion.div>
		</div>
	);
}
