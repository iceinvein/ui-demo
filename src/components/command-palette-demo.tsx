...
import {
  ArrowRight, FileText, Globe, Home, type LucideIcon, Moon, Palette,
  Search, Settings, Sun, Users, Zap,
} from "lucide-react";
import { useCallback, useEffect, useMemo, useRef, useState } from "react";

type PaletteItem = {
  id: string; label: string; icon: LucideIcon; group: string; shortcut?: string;
};
...
export function CommandPaletteDemo() {
  ...
  const openPalette = useCallback(() => {
    setOpen(true); setQuery(""); setActiveIndex(0); setLastAction(null);
  }, []);
  ...
  const selectItem = useCallback(
    (item: PaletteItem) => { setLastAction(item.label); closePalette(); },
    [closePalette],
  );
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    else if (e.key === "ArrowUp") { e.preventDefault(); setActiveIndex((i) => (i - 1 + flatItems.length) % flatItems.length); }
    else if (e.key === "Enter" && flatItems[activeIndex]) { selectItem(flatItems[activeIndex]); }
  ...
  return (
    <div className="flex min-h-[500px] flex-col items-center justify-center gap-6 p-8">
      <motion.div className="flex flex-col items-center gap-2" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-default-500 text-sm">Press ⌘K or click the button below</p>
      </motion.div>
      ...
      >
        ...
        <kbd className="ml-4 rounded-md border border-default-200 bg-default-100 px-2 py-0.5 font-mono text-default-400 text-xs">⌘K</kbd>
      </motion.button>
      ...
      <AnimatePresence>
        {lastAction && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            ...
            <ArrowRight className="h-4 w-4" /><span>Executed: {lastAction}</span>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {open && (
          <>
            <motion.div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm"
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={closePalette} />
            <motion.div
            ...
            >
              <div className="flex items-center gap-3 border-default-200/60 border-b px-4 py-3">
                ...
                <kbd className="rounded border border-default-200 bg-default-100 px-1.5 py-0.5 font-mono text-default-400 text-xs">ESC</kbd>
              </div>

              <div className="max-h-72 overflow-y-auto py-2">
                {filtered.length === 0 ? (
                  <p className="px-4 py-8 text-center text-default-400 text-sm">No results found</p>
                ) : (
                  groups.map(([group, groupItems]) => (
                    <div key={group}>
                      <p className="px-4 pt-2 pb-1 text-default-400 text-xs font-medium uppercase tracking-wider">{group}</p>
                      {groupItems.map((item) => {
                        const globalIdx = flatItems.indexOf(item);
                        const isActive = globalIdx === activeIndex;
                        const Icon = item.icon;
                        return (
                          <motion.button key={item.id} type="button" onClick={() => selectItem(item)}
                            onMouseEnter={() => setActiveIndex(globalIdx)}
                            className={`flex w-full items-center gap-3 px-4 py-2.5 text-left text-sm transition-colors ${
                              isActive ? "bg-default-100 text-default-900" : "text-default-600 hover:bg-default-50"
                            }`}
                          ...
                          >
                            ...
                            {item.shortcut && <span className="text-default-400 text-xs font-mono">{item.shortcut}</span>}
                          </motion.button>
                        );
                      })}
                    </div>
              ...
              <div className="flex items-center gap-4 border-default-200/60 border-t px-4 py-2 text-default-400 text-xs">
                <span className="flex items-center gap-1"><kbd className="rounded border border-default-200 bg-default-100 px-1 font-mono">↑↓</kbd> navigate</span>
                <span className="flex items-center gap-1"><kbd className="rounded border border-default-200 bg-default-100 px-1 font-mono">↵</kbd> select</span>
                <span className="flex items-center gap-1"><kbd className="rounded border border-default-200 bg-default-100 px-1 font-mono">esc</kbd> close</span>
              </div>
            </motion.div>
          </>
        )}
